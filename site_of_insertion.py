...
input is a text file of BAM files, one per line

will parse BAM files, open the Bed and BAM file, find set and parse to make
...
def parse( bam, bedFile ):
    ...
    out_table = bam.replace("lessThan25.bam", "_insertion_site_table.txt")
    with open(out_table, 'w') as o:
        for each in final_count_list:
            o.write(each)
    
    out_wig = bam.replace("lessThan25.bam", "_insertion_site.wig")            
...
def main():
    ...
    cmdparser = argparse.ArgumentParser(description="Parsing BAM and mpileup for transposon insertion locations.",
                                        usage='%(prog)s -f <list of BAM files to process> [optional arguments: -d]', prog='site_of_insertion.py'  )                                  
    ...
    if cmdResults['DETAIL']:
        ...
        print("\nInput : Text file with list of BAM files, one per line.")
        sys.exit(1)
